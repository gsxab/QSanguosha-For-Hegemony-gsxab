...
{
    m_isHovered = true;
    if (_transferable && _transferButton->isEnabled() && !RoomSceneInstance->getDasboard()->currentSkill())
...
void CardItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    if (!_m_isUnknownGeneral) {
        ...
        if (!card && objectName() != "unknown" && !general) {
            ...
            QString card_name = objectName();
            if (RoomSceneInstance->battle_started && card && card->isKindOf("Peach"))
                card_name = "aozhan";
            painter->drawPixmap(G_COMMON_LAYOUT.m_cardMainArea,
                G_ROOM_SKIN.getCardMainPixmap(card_name));
        } else {
            painter->drawPixmap(G_COMMON_LAYOUT.m_cardMainArea,
